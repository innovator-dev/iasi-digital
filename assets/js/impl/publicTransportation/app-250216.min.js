...
callback:c=>{c.forEach(a=>{this.vehicleRoutes[a.route_id]={name:a.route_short_name,long:a.route_long_name,type:a.route_type,color:a.route_color}});b&&b()}})}render(b){0===Object.keys(this.vehicleTrips).length&&this.getTripsData();this.hasData()?this.data.forEach(c=>{const a=new Vehicle(c,this.vehicleRoutes,this.vehicleTrips);if(a.latitude&&a.longitude&&a.routeId&&a.tripId&&a.routeName&&60>a.lastUpdateFriendly)if(this.markers[a.id])this.markers[a.id]._ref.position={lat:parseFloat(c.latitude),lng:parseFloat(c.longitude)},
!this.markers[a.id].isVisible&&this.markers[a.id]._ref&&(this.markers[a.id]._ref.setMap(CityApp.data.map._ref),this.markers[a.id].isVisible=!0);else{let [d,e]=this.routes[a.routeNumber]?this.routes[a.routeNumber]:this.routes[0];c=document.createElement("div");c.innerText=a.routeNumber;c=new google.maps.marker.PinElement({glyphColor:e,background:d,borderColor:d,glyph:c});c=new google.maps.marker.AdvancedMarkerElement({position:{lat:parseFloat(a.latitude),lng:parseFloat(a.longitude)},map:CityApp.data.map._ref,
title:a.label,content:c.element});this.markers[a.id]={id:a.id,type:a.routeType,route:a.routeNumber,_ref:c,isVisible:!0};this.markers[a.id]._ref.addListener("click",()=>{this.selectedMarker=this.markers[a.id]._ref;CityApp.mapUtils("closePopup");CityApp.mapUtils("createPopup",{title:a.routeName,titleLabel:this.markers[a.id].route,titleLabelClass:["route",`route-${this.markers[a.id].route}`],content:`<ul><li><strong>${CityApp.config.labels["transportation.direction"]}: ${a.tripHeadSign}</strong></li><li title="${a.lastUpdate}">${CityApp.config.labels["transportation.lastUpdate"]}: ${CityApp.config.labels["transportation.tkTimeAgo"].replace("{{TIME}}",
