...
airQuality:d.querySelector('input[type=checkbox][name="toggle.environment.airQuality"]'),wasteCollection:d.querySelector('input[type=checkbox][name="toggle.environment.wasteCollection"]')};d.classList.remove("hide");d.querySelectorAll(".btn-toggle").forEach(l=>{const b=l.dataset;b.set&&(p[b.set]={app:null,watcher:null,loaded:!1,visible:!1,hasData:!1,updated:null,data:{},markers:[],selectedMarker:null},b.source&&(k(b.source,()=>{p[b.set].loaded=!0;p[b.set].app.getData()?(p[b.set].hasData=!0,p[b.set].app.init()):
c.controls[b.set].setAttribute("disabled","disabled")}),l.addEventListener("click",()=>{c.controls.spinner.classList.remove("hide");c.controls[b.set].checked?p[b.set].app.show():p[b.set].app.hide();c.controls.spinner.classList.add("hide")})),"trafficLayer"===b.set&&l.addEventListener("click",()=>{c.controls.spinner.classList.remove("hide");c.controls[b.set].checked?(p[b.set].app=new google.maps.TrafficLayer,p[b.set].app.setMap(c.ref),p[b.set].visible=!0):(p[b.set].visible=!1,p[b.set].app.setMap(null),
p[b.set].app=null);c.controls.spinner.classList.add("hide")}))})}});r()}}})();(()=>{const k=document.querySelector("body.page").dataset;k.bind&&(app.config=JSON.parse(atob(k.bind)));app.api("config.json").then(m=>m.ok?m.json():!1).then(m=>{app.config=Object.assign(app.config,m)}).catch(m=>{});app.render()})();
